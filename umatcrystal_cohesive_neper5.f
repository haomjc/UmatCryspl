...
C
      ...
      CHARACTER*100 FILE,FILE1,FILE2
C
      DIMENSION TIME(2)
      
      
      integer nmax_p
      parameter (nmax_p=5E5)

      real*8 FP_0(nmax_p),LOCAL_FP0(nmax_p)
      real*8 LOCALGAMMAMAX(nmax_p),LOCALGAMMAMIN(nmax_p),
     1 LOCALTAUMAX(nmax_p),LOCALTAUMIN(nmax_p),LOCALEPSILONMAX(nmax_p),
     2 LOCALEPSILONMIN(nmax_p),LOCALSIGMAMAX(nmax_p),LOCALFIP(nmax_p),
     3 Damage_rate(nmax_p),LOCALDAMAGE(nmax_p),
     4 FIPMAX,Damage_critical,N_cycle
      integer FLAG_CYCLE,FLAG_XIT

      COMMON LOCALGAMMAMAX,LOCALGAMMAMIN,
     1 LOCALTAUMAX,LOCALTAUMIN,LOCALEPSILONMAX,
     2 LOCALEPSILONMIN,LOCALSIGMAMAX,LOCALFIP,
     3 Damage_rate,LOCALDAMAGE,
     4 N_cycle,FIPMAX,Damage_critical,FLAG_CYCLE,FLAG_XIT
C     NINALL = 2000000 
C
...
C     积分点的总数量
      ...
      real*8 J_fc,m_fc,delta_Df,C_fc
...
C      call MutexLock( 1 )
      IF(LOP.eq.0)THEN
         FILE1='E:\temp\UmatCryspl\texture_init_230822_3.dat'
         OPEN(91,FILE=FILE1,STATUS='UNKNOWN',ACCESS='APPEND')
         FILE2='E:\temp\UmatCryspl\texture_end_230822_3.dat'
         OPEN(92,FILE=FILE2,STATUS='UNKNOWN',ACCESS='APPEND')
      ENDIF

      IF(LOP.eq.6.and.FLAG_CYCLE.eq.1)THEN
         LOCALFIP = (LOCALEPSILONMAX-LOCALEPSILONMIN)/2.0*LOCALSIGMAMAX
     1 + J_fc*(LOCALGAMMAMAX-LOCALGAMMAMIN)*(LOCALTAUMAX-LOCALTAUMIN)
         FIPMAX = MAXVAL(LOCALFIP)

C         Damage_rate = (ABS(LOCALFIP - FP_0))**m_fc/C_fc
         LOCAL_FP0 = ABS(LOCALFIP - FP_0)
C         Damage_rate = LOCAL_FP0*LOCAL_FP0/C_fc
         Damage_rate = LOCAL_FP0**m_fc/C_fc
C         Damage_rate = LOCAL_FP0C_fc

         Damage_critical = MAXVAL(Damage_rate)
         d_N_cycle = delta_Df/Damage_critical
         LOCALDAMAGE = LOCALDAMAGE + Damage_rate*d_N_cycle

         N_cycle = N_cycle + d_N_cycle

      END IF

C      call MutexUnlock( 1 )
...
C      ELS = 335232
C      NOELN = (NOEL-ELS)*3 + NPT

      NOELN = NOEL*3 + NPT

      IF(NOELN.gt.nmax_p)then
         NOELN = MOD(NOELN,nmax_p)
      ENDIF
     ...
     1 LACCFLA)
      ...
      GAMMA_eq = sqrt(E13**2. + E23**2.)
      TAU_eq = sqrt(S13**2. + S23**2.)
      EPSILON = E33
      SIGMA = S33
...
C      call MutexLock( 1 )
      ...
      if(KINC.eq.1)then
         LOCALGAMMAMAX(NOELN) = GAMMA_eq
         LOCALGAMMAMIN(NOELN) = GAMMA_eq
         LOCALTAUMAX(NOELN) = TAU_eq
         LOCALTAUMIN(NOELN) = TAU_eq
         LOCALEPSILONMAX(NOELN) = EPSILON
         LOCALEPSILONMIN(NOELN) = EPSILON
         LOCALSIGMAMAX(NOELN) = ABS(SIGMA)
      else


         if(GAMMA_eq>LOCALGAMMAMAX(NOELN))then
            LOCALGAMMAMAX(NOELN) = GAMMA_eq
         endif

         if(GAMMA_eq<LOCALGAMMAMIN(NOELN))then
            LOCALGAMMAMIN(NOELN) = GAMMA_eq
         endif      


         if(TAU_eq>LOCALTAUMAX(NOELN))then
            LOCALTAUMAX(NOELN) = TAU_eq
         endif

         if(TAU_eq<LOCALTAUMIN(NOELN))then
            LOCALTAUMIN(NOELN) = TAU_eq
         endif  


         if(EPSILON>LOCALEPSILONMAX(NOELN))then
            LOCALEPSILONMAX(NOELN) = EPSILON
         endif

         if(EPSILON<LOCALEPSILONMIN(NOELN))then
            LOCALEPSILONMIN(NOELN) = EPSILON
         endif   

         if(ABS(SIGMA)>LOCALSIGMAMAX(NOELN))then
            LOCALSIGMAMAX(NOELN) = ABS(SIGMA)
         endif

      endif

         IF(LOCALDAMAGE(NOELN)>0.99.and.KINC.eq.1)THEN
            FLAG_XIT=1

         ELSE IF(LOCALDAMAGE(NOELN)>0.99.and.KINC.ne.1)THEN
            CALL XIT
         ELSE

            FIELD(1) = LOCALDAMAGE(NOELN)
            STATEV(1) = LOCALDAMAGE(NOELN)
C            STATEV(1) = GAMMA_eq
            STATEV(2) = LOCALFIP(NOELN)
C            STATEV(2) = LOCALGAMMAMIN(NOELN)
            STATEV(3) = N_cycle
...
C
     ...
     2 LOCALEPSILONMIN(nmax_p),LOCALSIGMAMAX(nmax_p),LOCALFIP(nmax_p),
     3 Damage_rate(nmax_p),LOCALDAMAGE(nmax_p),
     4 FIPMAX,Damage_critical,N_cycle,gamma_qd,judge_Miu
      integer FLAG_CYCLE,FLAG_XIT
...
c     受力方向
      ...
      T_USER(3) = 0.
c     受力大小
      Miu_max = 1831.05*0.05
      a_ellip = 0.2097
      b_ellip = 0.1401
      x_c = 0. + move
      z_c = 0.5
      
      gamma_qd = 3.1415926/2 - 1.7982
C      judge_p_ij = 1-((pis*cos(gamma_qd)+pj*sin(gamma_qd))/b(k,h))^2-((-pis*sin(gamma_qd)+pj*cos(gamma_qd))/a(k,h))^2
      judge_Miu = 1-(((x-x_c)*cos(gamma_qd)+(z-z_c)*
     1 sin(gamma_qd))/b_ellip)**2
     2 -((-(x-x_c)*sin(gamma_qd)+(z-z_c)*cos(gamma_qd))/a_ellip)**2

      if(judge_Miu>=0.)then
         if(FLAG_CYCLE.eq.1)then
            alpha = Miu_max*sqrt(judge_Miu)
         else
            alpha = time(1)*Miu_max*sqrt(judge_Miu)
         endif
      
C      if(x>=12.and.x<=18)then
C          if (z <= move+18 .and. z >= move+12)then
C              alpha = 10.0
C          else
C              alpha = 0.0
C          end if
...
C
     ...
     2 LOCALEPSILONMIN(nmax_p),LOCALSIGMAMAX(nmax_p),LOCALFIP(nmax_p),
     3 Damage_rate(nmax_p),LOCALDAMAGE(nmax_p),
     4 FIPMAX,Damage_critical,N_cycle,gamma_qd,judge_P
      integer FLAG_CYCLE,FLAG_XIT
...
C      call MutexUnlock( 1 )
      ...
      gamma_qd = 3.1415926/2 - 1.7982
C      judge_p_ij = 1-((pis*cos(gamma_qd)+pj*sin(gamma_qd))/b(k,h))^2-((-pis*sin(gamma_qd)+pj*cos(gamma_qd))/a(k,h))^2
      judge_P = 1-(((x-x_c)*cos(gamma_qd)+(z-z_c)*
     1 sin(gamma_qd))/b_ellip)**2
     2 -((-(x-x_c)*sin(gamma_qd)+(z-z_c)*cos(gamma_qd))/a_ellip)**2

      if(judge_P>=0.)then
         if(FLAG_CYCLE.eq.1)then
            F = P_max*sqrt(judge_P)
         else
            F = time(1)*P_max*sqrt(judge_P)
         endif
...
c
     ...
     4 N_cycle,FIPMAX,Damage_critical,FLAG_CYCLE,FLAG_XIT
c
      CHARACTER*100 CMNAME,FILE,FILE1,FILE2
      EXTERNAL F
...
C 	    FILE1='E:\temp\UmatCryspl\temp.dat'
C      OPEN(91,FILE=FILE1,STATUS='UNKNOWN',ACCESS='APPEND')
      DO J=1,6
         DO I=1,6
            DLOCAL(I,J)=0.
         END DO
...
C-----  Total number of sets of slip systems: NSET
      NSET=NINT(PROPS(25))
      IF (NSET.LT.1) THEN
C         WRITE (6,*) '***ERROR - zero sets of slip systems'
         STOP
      ELSE IF (NSET.GT.3) THEN
C         WRITE (6,*) 
C     2     '***ERROR - more than three sets of slip systems'
         STOP
      END IF
...
C            WRITE (6,*) 
C     2 '***ERROR - parameter ND chosen by the present user is less than
C     3             the total number of slip systems NSLPTL'
            STOP
         END IF
...
C
      ...
      DO I=1,NSLPTL
         ...
         GSLIP=STATEV(I)
         X=TAUSLP/GSLIP
         TERM2=TERM1*DFDXSP(I)/GSLIP!-------------theta*DTime*DXDt(求偏导)
         TERM3=TERM1*X*DFDXSP(I)/GSLIP!-----------theta*DTime*DXDg(求偏导)

         DO J=1,NSLPTL
            TERM4=0.
            DO K=1,6
               TERM4=TERM4+DDEMSD(K,I)*SLPDEF(K,J)!------DDEMSD是体现应力客观性的表达式,SLPDEF是滑移系之间的Schmid因子
            END DO
...
C-----  Check if the iteration solution converges
     ...
     2                FSLIP1(ID)-DGAMMA(ID)
               IF (ABS(RESIDU).GT.GAMERR) IDBACK=1!如果残差大于阈值，就将IDBACK值设为1，否则设为0
            END DO
         END DO

         IF (IDBACK.NE.0.AND.NITRTN.LT.ITRMAX) THEN!如果IDBACK值不为0，且迭代次数小于最大迭代次数，就继续迭代
...
C----------------------------------------------------------------------
 ...
 100   FORMAT (1X,I3,I7,I2,F10.4,F10.4,F10.4,F30.22,F30.22,F30.22,
     1                      F30.22,F30.22,F30.22) 
      endif

      if(kinc==1.and.FLAG_XIT==1) then
      write (92,101)  kinc, noel, npt, coords(1),coords(2),coords(3),
     2            (statev(IDNOR),IDNOR=3*NSLPTL+1,3*NSLPTL+3),
     3            (statev(IDDIR),IDDIR=6*NSLPTL+1,6*NSLPTL+3)
   
 101   FORMAT (1X,I3,I7,I2,F10.4,F10.4,F10.4,F30.22,F30.22,F30.22,
     1                      F30.22,F30.22,F30.22) 
      endif
...
C         WRITE (6,*) 
C     2      '***ERROR - angles between two vectors are not the same'
         STOP
      END IF
...
C
           ...
           IF (SUM1.EQ.0.) THEN
C              WRITE (6,*) '***ERROR - first vector is zero'
              STOP
           ELSE
              DO I=1,3
                 C(I,1)=A(I)/SUM1
           ...
           IF (SUM2.EQ.0.) THEN
C              WRITE (6,*) '***ERROR - second vector is zero'
              STOP
           ELSE
              DO I=1,3
                 C(I,2)=B(I)/SUM2
...
C              WRITE (6,*) 
C     2           '***ERROR - first and second vectors are parallel'
               STOP
            END IF
...
C     I1=I2=I3=0
      IF (I3.EQ.0) THEN 
C         WRITE (6,*) '***ERROR - slip direction is [000]'
         STOP
...
C     mode, e.g. (1 -2 1), J1=1, J2=1 and J3=2.  J1<=J2<=J3.
      ...
      IF (J3.EQ.0) THEN 
C         WRITE (6,*) '***ERROR - slip plane is [000]'
         STOP
...
C      WRITE (6,*) '          '
      ...
      IF (NSLIP.EQ.0) THEN
C         WRITE (6,*) 
C     *      'There is no slip direction normal to the slip planes!'
         STOP
